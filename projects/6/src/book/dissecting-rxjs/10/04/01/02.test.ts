...
describe('src/book/dissecting-rxjs/10/04/01/02.ts', () => {
  ...
  // 默认情况下, multicast() 返回一个 ConnectableObservable 对象, 我们需要手动调用 ConnectableObservable.connect() 方法来订阅上游的 cold observable,
  ...
  it('should work', () => {
    ...
    const observable$ = (source$.pipe(multicast<number>(new Subject())) as ConnectableObservable<number>).refCount();
  ...
  it('should work 02', () => {
    ...
    const source$ = interval(1000).pipe(take(3));
    const observable$ = source$.pipe(multicast<number>(new Subject()), refCount());
